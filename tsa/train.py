...
def train(train_iter, test_iter, model, criterion, optimizer, config, ts):
    ...
    tb_writer_train = SummaryWriter(
        logdir=config.general.output_dir, filename_suffix="train"
    )
    tb_writer_test = SummaryWriter(
        logdir=config.general.output_dir, filename_suffix="test"
    )
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=config.training.lrs_step_size, gamma=0.5
    )
    ...
    for epoch in tqdm(range(config.training.num_epochs), unit="epoch"):
        for i, batch in tqdm(
            enumerate(train_iter), total=len(train_iter), unit="batch"
        ):
            model.train()
            optimizer.zero_grad()
            ...
            if (i + 1) % config.training.gradient_accumulation_steps == 0:
                ...
                if global_step % config.general.logging_steps == 0:
                    if config.general.eval_during_training:
                        results = evaluate(
                            test_iter, criterion, model, config, ts
                        )
                        for key, val in results.items():
                            tb_writer_test.add_scalar(
                                "eval_{}".format(key), val, global_step
                            )

                    tb_writer_train.add_scalar(
                        "train_loss",
                        (train_loss - logging_loss)
                        / config.general.logging_steps,
                        global_step,
                    )
                    tb_writer_train.add_scalar(
            ...
            if global_step % config.general.save_steps == 0:
                torch.save(
                    ...
                    "{}/checkpoint-{}.ckpt".format(
                        config.general.output_dir, global_step
                    ),
                )
