...
class Encoder(nn.Module):
    def __init__(self, config, input_size: int):
        ...
        self.lstm = nn.LSTM(
            input_size=input_size, hidden_size=config["hidden_size_encoder"]
        )
...
class AttnEncoder(nn.Module):
    ...
    def forward(self, input_data: torch.Tensor):
        ...
        attentions, input_encoded = (
            Variable(
                torch.zeros(input_data.size(0), self.seq_len, self.input_size)
            ),
            Variable(
                torch.zeros(input_data.size(0), self.seq_len, self.hidden_size)
            ),
        ...
        for t in range(self.seq_len):
            ...
            a_t = self.softmax(e_t.view(-1, self.input_size)).to(
                device
            )  # (bs, input_size)

            weighted_input = torch.mul(
                a_t, input_data[:, t, :].to(device)
            )  # (bs * input_size)
...
class Decoder(nn.Module):
    def __init__(self, config):
        ...
        self.lstm = nn.LSTM(
            1, config["hidden_size_decoder"], bidirectional=False
        )
        self.fc = nn.Linear(
            config["hidden_size_decoder"], config["output_size"]
        )
...
class AttnDecoder(nn.Module):
    def __init__(self, config):
        ...
        self.fc = nn.Linear(
            self.encoder_hidden_size + self.out_feats, self.out_feats
        )
    ...
    def forward(self, input_encoded: torch.Tensor, y_history: torch.Tensor):
        ...
        context = Variable(
            torch.zeros(input_encoded.size(0), self.encoder_hidden_size)
        )
        ...
        for t in range(self.seq_len):
            ...
            y_tilde = self.fc(
                torch.cat(
                    (context.to(device), y_history[:, t].to(device)), dim=1
                )
            )  # (batch_size, out_size)
...
class AutoEncForecast(nn.Module):
    def __init__(self, config, input_size):
        ...
        self.decoder = (
            AttnDecoder(config).to(device)
            if config["temporal_att"]
            else Decoder(config).to(device)
        )
